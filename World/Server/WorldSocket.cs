...
namespace HermesProxy.World.Server
{
    public partial class WorldSocket : SocketBase, BnetServices.INetwork
    {
        ...
        public void SendPacket(ServerPacket packet)
        {
            ...
            // Check SkipSend AFTER WritePacketData (flag is set during Write())
            if (packet.SkipSend)
                return;
            if (GetSession() != null)
            ...
            // Don't send any packet with opcode 0 to the client — the client can't handle it
            if (opcode == 0)
            {
                Log.PrintNet(LogType.Warn, LogNetDir.P2C, $"Dropping packet with opcode 0 (universal={universalOpcode}, size={data.Length})");
                _sendMutex.ReleaseMutex();
                return;
            }

            Log.PrintNet(LogType.Debug, LogNetDir.P2C, $"Sending opcode {universalOpcode} ({(uint)opcode}), size={data.Length}.");

            ByteBuffer buffer = new();

            int packetSize = data.Length;
            if (packetSize > 0x400 && _worldCrypt.IsInitialized && ModernVersion.ExpansionVersion < 3)
            {
        ...
        void HandleAuthSessionCallback(AuthSession authSession)
        {
            ...
            if (platformSeed == null || !TrySeed(platformSeed))
            {
                ...
                {
                    // TODO: Arctium's auth seed patch fails for build 54261 because CryptoRsaModulus
                    // pattern is not found. Skip seed check for now to allow testing.
                    Log.Print(LogType.Warn, $"WorldSocket.HandleAuthSession: Seed mismatch for account: {GetSession().GameAccountInfo.Id} ('{authSession.RealmJoinTicket}') - BYPASSING for testing");
                }
            }
        ...
        public void SendFeatureSystemStatusGlueScreen()
        {
            ...
            features.MaximumExpansionLevel = 2; // WotLK = expansion 2
            features.Unk14 = true;
        ...
        public void SendSeasonInfo()
        {
            ...
            Framework.Logging.Log.Print(Framework.Logging.LogType.Debug, $"SeasonInfo opcode resolved to: {resolved} (0x{(uint)resolved:X4})");
