...
namespace HermesProxy.World.Server.Packets
{
    ...
    public class FeatureSystemStatus : ServerPacket
    {
        ...
        {
            // WotLK Classic 3.4.3 has a DIFFERENT format from TBC Classic 2.5.3
        ...
        // WotLK Classic 3.4.3 format - matches TC TDB343.24081 exactly
        private void WriteWotLK()
        {
            _worldPacket.WriteUInt8(ComplaintStatus);

            // NO ScrollOfResurrection fields in 3.4.3!
            _worldPacket.WriteUInt32(CfgRealmID);
            _worldPacket.WriteInt32(CfgRealmRecID);
            ...
            _worldPacket.WriteUInt32(RAFSystem.MaxRecruitmentUses);
            _worldPacket.WriteUInt32(RAFSystem.DaysInCycle);
            _worldPacket.WriteUInt32(0); // RAFSystem.Unknown1007

            // NO TwitterPostThrottle fields in 3.4.3!
            _worldPacket.WriteUInt32(TokenPollTimeSeconds);
            _worldPacket.WriteUInt32(KioskSessionMinutes);
            _worldPacket.WriteInt64(TokenBalanceAmount);
            ...
            _worldPacket.WriteInt16(MaxPlayerNameQueriesPerPacket);
            _worldPacket.WriteInt16(PlayerNameQueryTelemetryInterval);
            _worldPacket.WriteInt32(0); // PlayerNameQueryInterval (Duration)

            foreach (var rulePair in GameRuleValues)
                rulePair.Write(_worldPacket);

            // Bits - exact TC 3.4.3 order
            _worldPacket.WriteBit(VoiceEnabled);
            _worldPacket.WriteBit(EuropaTicketSystemStatus != null);
            // NO ScrollOfResurrectionEnabled bit!
            _worldPacket.WriteBit(BpayStoreEnabled);
            _worldPacket.WriteBit(BpayStoreAvailable);
            _worldPacket.WriteBit(BpayStoreDisabledByParentalControls);
            _worldPacket.WriteBit(ItemRestorationButtonEnabled);
            ...
            _worldPacket.WriteBit(false); // WarGamesEnabled
            _worldPacket.WriteBit(false); // ContentTrackingEnabled
            _worldPacket.WriteBit(false); // IsSellAllJunkEnabled
            _worldPacket.WriteBit(false); // IsGroupFinderEnabled
            _worldPacket.WriteBit(true);  // IsLFDEnabled

            _worldPacket.WriteBit(true);  // IsLFREnabled
            _worldPacket.WriteBit(true);  // IsPremadeGroupEnabled
            // NO BattlegroundsEnabled/RaceClassExpansionLevels in 3.4.3!
            ...
            // NO RaceClassExpansionLevels section in 3.4.3!
    ...
    public class FeatureSystemStatusGlueScreen : ServerPacket
    {
        ...
        void WriteWotlk343()
        {
            // WotLK Classic 3.4.3 format (30+ bits, different from TBC)
            // Byte 1 (8 bits)
            _worldPacket.WriteBit(BpayStoreEnabled);
            _worldPacket.WriteBit(BpayStoreAvailable);
            _worldPacket.WriteBit(BpayStoreDisabledByParentalControls);
            ...
            _worldPacket.WriteBit(false); // Unk
            _worldPacket.WriteBit(false); // IsRealmHiddenAlert (if true, need ReadBits(11))
            _worldPacket.FlushBits();
            ...
            _worldPacket.WriteInt64(TokenBalanceAmount);
            _worldPacket.WriteInt32(MaxCharactersPerRealm);
            _worldPacket.WriteInt32(LiveRegionCharacterCopySourceRegions.Count); // liveRegionCharacterCopySourceRegionsCount
            _worldPacket.WriteUInt32(BpayStoreProductDeliveryDelay);
            _worldPacket.WriteInt32(ActiveCharacterUpgradeBoostType);
            _worldPacket.WriteInt32(ActiveClassTrialBoostType);
            _worldPacket.WriteInt32(MinimumExpansionLevel);
            _worldPacket.WriteInt32(MaximumExpansionLevel);
            _worldPacket.WriteInt32(ActiveSeason);
            _worldPacket.WriteInt32(GameRuleValues.Count); // gameRuleValuesCount
            _worldPacket.WriteInt16(MaxPlayerNameQueriesPerPacket);
            _worldPacket.WriteInt16(PlayerNameQueryTelemetryInterval);
            ...
            // LaunchETA not written (bit was false)
            ...
            // DebugTimeEvents not written (count was 0)
        }
