...
namespace HermesProxy.World.Server.Packets
{
    ...
    public class UpdateObject : ServerPacket
    {
        ...
        // Buffer pending updates during login until player object arrives (WotLK 3.4.3)
        [ThreadStatic] private static List<ObjectUpdate> _pendingLoginUpdates;
        [ThreadStatic] private static List<WowGuid128> _pendingLoginDestroys;
        [ThreadStatic] private static bool _playerObjectSent;

        public static void ResetLoginBuffer()
        {
            _pendingLoginUpdates = new List<ObjectUpdate>();
            _pendingLoginDestroys = new List<WowGuid128>();
            _playerObjectSent = false;
        }

        public override void Write()
        {
            // For WotLK Classic: buffer updates until player object arrives to avoid DC
            if (ModernVersion.ExpansionVersion >= 3 && !_playerObjectSent)
            {
                if (_pendingLoginUpdates == null) ResetLoginBuffer();
                ...
                {
                    _pendingLoginUpdates.AddRange(ObjectUpdates);
                    _pendingLoginDestroys.AddRange(DestroyedGuids);
                    Framework.Logging.Log.Print(Framework.Logging.LogType.Debug, $"[UpdateObject] Buffering {ObjectUpdates.Count} updates (total: {_pendingLoginUpdates.Count})");
                    // Set SkipSend flag - don't send anything for buffered packets
                    SkipSend = true;
                    return;
                }
                ...
                if (hasPlayer)
                {
                    var merged = new List<ObjectUpdate>(_pendingLoginUpdates);
                    merged.AddRange(ObjectUpdates);
                    ObjectUpdates = merged;
                    DestroyedGuids.AddRange(_pendingLoginDestroys);
                    _playerObjectSent = true;
                    Framework.Logging.Log.Print(Framework.Logging.LogType.Debug, $"[UpdateObject] Merged {_pendingLoginUpdates.Count} buffered + sending {ObjectUpdates.Count} total updates");
                }
            }

            MapID = (ushort)_gameState.CurrentMapId;

            // Filter out GameObjects if debug flag is set
            if (Objects.Version.V3_4_3_54261.ObjectUpdateBuilder.DEBUG_SKIP_GAMEOBJECTS)
                ObjectUpdates.RemoveAll(u => u.Guid.GetObjectType() == Enums.ObjectType.GameObject);
            // Skip Transport GameObjects for 3.4.3 (format TBD)
            if (ModernVersion.ExpansionVersion >= 3)
                ObjectUpdates.RemoveAll(u => u.Guid.GetHighType() == Enums.HighGuidType.Transport);

            // Count AFTER filtering so header matches actual data
            ...
            Framework.Logging.Log.Print(Framework.Logging.LogType.Debug, $"[UpdateObject] Writing {ObjectUpdates.Count} updates, {DestroyedGuids.Count} destroyed, {OutOfRangeGuids.Count} OOR, map={MapID}");
            ...
            Framework.Logging.Log.Print(Framework.Logging.LogType.Debug, $"[UpdateObject] Data block size={bytes.Length}, first 64 bytes: {BitConverter.ToString(bytes, 0, Math.Min(64, bytes.Length))}");
            // Dump full packet to file for debugging
            if (bytes.Length > 1000)
            {
                try
                {
                    string dumpPath = System.IO.Path.Combine(System.AppDomain.CurrentDomain.BaseDirectory, "updateobject_dump.bin");
                    // Write full packet: count(4) + map(2) + buffer(bit+size+data)
                    using (var fs = new System.IO.FileStream(dumpPath, System.IO.FileMode.Create))
                    {
                        fs.Write(BitConverter.GetBytes(NumObjUpdates), 0, 4);
                        fs.Write(BitConverter.GetBytes(MapID), 0, 2);
                        byte[] bufferData = buffer.GetData();
                        // Write the bit byte for OOR section
                        fs.WriteByte(0); // no OOR
                        fs.Write(BitConverter.GetBytes(bytes.Length), 0, 4);
                        fs.Write(bytes, 0, bytes.Length);
                    }
                    Framework.Logging.Log.Print(Framework.Logging.LogType.Debug, $"[UpdateObject] Dumped {bytes.Length} bytes to {dumpPath}");
                }
                catch { }
            }
