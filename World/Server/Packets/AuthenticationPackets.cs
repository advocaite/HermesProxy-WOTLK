...
namespace HermesProxy.World.Server.Packets
{
    ...
    class AuthResponse : ServerPacket
    {
        ...
        public override void Write()
        {
            ...
            if (SuccessInfo != null)
            {
                ...
                foreach (var raceClassAvailability in SuccessInfo.AvailableClasses)
                {
                    ...
                    foreach (var classAvailability in raceClassAvailability.Classes)
                    {
                        ...
                        if (ModernVersion.ExpansionVersion >= 3)
                            _worldPacket.WriteUInt8(0); // MinActiveExpansionLevel (WotLK Classic 3.4.3+)
                    }
                }
                ...
                if (ModernVersion.ExpansionVersion >= 3)
                    _worldPacket.WriteBit(false); // HasNewBuildKeys (WotLK Classic 3.4.3+)
                _worldPacket.FlushBits();
    ...
    class EnterEncryptedMode : ServerPacket
    {
        ...
        // RSA mode seeds (TBC Classic and older)
        static byte[] EnableEncryptionSeedRSA = { 0x90, 0x9C, 0xD0, 0x50, 0x5A, 0x2C, 0x14, 0xDD, 0x5C, 0x2C, 0xC0, 0x64, 0x14, 0xF3, 0xFE, 0xC9 };

        // Ed25519 mode seeds (WotLK Classic 3.4.x and newer)
        static byte[] EnableEncryptionSeedEd25519 = {
        ...
        static byte[] EnableEncryptionContext = {
            0xA7, 0x1F, 0xB6, 0x9B, 0xC9, 0x7C, 0xDD, 0x96, 0xE9, 0xBB, 0xB8, 0x21, 0x39, 0x8D, 0x5A, 0xD4
        };
        // Ed25519 private key (from TrinityCore - matches Arctium's CryptoEdPublicKey)
        static byte[] Ed25519PrivateKey = {
        ...
        public override void Write()
        {
            if (ModernVersion.ExpansionVersion >= 3)
            {
                // WotLK Classic 3.4.x+ uses Ed25519 with HMAC-SHA512
                WriteEd25519();
            }
            else
            {
                // TBC Classic and older use RSA
                WriteRSA();
            }
        }
        ...
        void WriteEd25519()
        {
            // Step 1: HMAC-SHA256(EncryptionKey, Enabled byte, EnableEncryptionSeed) → 32 bytes
            // Note: WotLK Classic may use SHA256 like older clients, not SHA512 like retail
            HmacSha256 hash = new(EncryptionKey);
            hash.Process(BitConverter.GetBytes(Enabled), 1);
            hash.Finish(EnableEncryptionSeedRSA, 16); // Use same seed as RSA version
            byte[] toSign = hash.Digest;

            Framework.Logging.Log.Print(Framework.Logging.LogType.Debug, $"EnterEncryptedMode Ed25519: toSign={BitConverter.ToString(toSign, 0, 16)}...");

            // Step 2: Ed25519ctx sign with context (RFC 8032)
            var privateKeyParams = new Org.BouncyCastle.Crypto.Parameters.Ed25519PrivateKeyParameters(Ed25519PrivateKey, 0);
            var signer = new Org.BouncyCastle.Crypto.Signers.Ed25519ctxSigner(EnableEncryptionContext);
            signer.Init(true, privateKeyParams);
            signer.BlockUpdate(toSign, 0, toSign.Length);
            byte[] signature = signer.GenerateSignature();

            Framework.Logging.Log.Print(Framework.Logging.LogType.Debug, $"EnterEncryptedMode Ed25519: signature={BitConverter.ToString(signature, 0, 16)}... ({signature.Length} bytes)");

            // Step 3: Write packet: 64-byte signature + Enabled bit (no RegionGroup for WotLK Classic)
