...
namespace HermesProxy.World.Server.Packets
{
    ...
    public class EnumCharactersResult : ServerPacket
    {
        ...
        public override void Write()
        {
            ...
            {
                // WotLK header (correct for 3.4.3)
                _worldPacket.WriteBit(false); // IsTrialAccountRestricted
                ...
                _worldPacket.WriteUInt32(0); // RaceLimitDisablesCount
                ...
                // RaceLimitDisables (count=0)

                foreach (CharacterInfo charInfo in Characters)
                    charInfo.Write(_worldPacket); // Still using TBC CharacterInfo format (no WotLK changes)
            ...
            {
                // TBC Classic / Classic Era format
                _worldPacket.WriteBit(DisabledClassesMask.HasValue);
        ...
        public class CharacterInfo
        {
            public void Write(WorldPacket data)
            {
                ...
                // WotLK Classic expects 34 visual items, TBC/Classic expects 23
                ...
                for (int vi = 0; vi < visualItemCount; vi++)
                {
                    if (vi < VisualItems.Length)
                        ...
                        new VisualItemInfo().Write(data); // empty padding
                }
                ...
                // Dump first 40 bytes of this character entry for debugging
                ...
                // Dump LAST 30 bytes (where name is)
                ...
                Framework.Logging.Log.Print(Framework.Logging.LogType.Debug, $"CharacterInfo: name={Name} race={RaceId} class={ClassId} level={ExperienceLevel} visItems={visualItemCount} totalBytes={totalSize}");
                Framework.Logging.Log.Print(Framework.Logging.LogType.Debug, $"CharacterInfo LAST 30 bytes: {lastHex}");
            }
    ...
    public class PlayerLogin : ClientPacket
    {
        ...
        public override void Read()
        {
            ...
            try { UnkBit = _worldPacket.HasBit(); } catch { UnkBit = false; }
        }
