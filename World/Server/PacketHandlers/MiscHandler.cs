...
namespace HermesProxy.World.Server
{
    public partial class WorldSocket
    {
        ...
        // WotLK Classic client requests - stub handlers
        [PacketHandler(Opcode.CMSG_VIOLENCE_LEVEL)]
        void HandleViolenceLevel(ViolenceLevelPkt packet) { }

        [PacketHandler(Opcode.CMSG_REQUEST_PVP_REWARDS)]
        void HandleRequestPvpRewards(RequestPvpRewardsPkt packet) { }

        [PacketHandler(Opcode.CMSG_REQUEST_RATED_PVP_INFO)]
        void HandleRequestRatedPvpInfo(RequestRatedPvpInfoPkt packet) { }

        [PacketHandler(Opcode.CMSG_OVERRIDE_SCREEN_FLASH)]
        void HandleOverrideScreenFlash(OverrideScreenFlashPkt packet) { }

        [PacketHandler(Opcode.CMSG_QUEUED_MESSAGES_END)]
        void HandleQueuedMessagesEnd(QueuedMessagesEndPkt packet) { }

        [PacketHandler(Opcode.CMSG_BATTLE_PAY_GET_PRODUCT_LIST)]
        void HandleBattlePayGetProductList(BattlePayGetProductListPkt packet) { }

        [PacketHandler(Opcode.CMSG_BATTLE_PAY_GET_PURCHASE_LIST)]
        void HandleBattlePayGetPurchaseList(BattlePayGetPurchaseListPkt packet) { }

        [PacketHandler(Opcode.CMSG_GET_UNDELETE_CHARACTER_COOLDOWN_STATUS)]
        void HandleGetUndeleteCharacterCooldownStatus(GetUndeleteCharCooldownPkt packet) { }

        [PacketHandler(Opcode.CMSG_UPDATE_VAS_PURCHASE_STATES)]
        void HandleUpdateVasPurchaseStates(UpdateVasPurchaseStatesPkt packet) { }
        ...
        void HandleCalendarGetNumPending(CalendarGetNumPendingPkt packet) { }

        [PacketHandler(Opcode.CMSG_GUILD_SET_ACHIEVEMENT_TRACKING)]
        void HandleGuildSetAchievementTracking(GuildSetAchievementTrackingPkt packet) { }

        [PacketHandler(Opcode.CMSG_QUERY_COUNTDOWN_TIMER)]
        void HandleQueryCountdownTimer(QueryCountdownTimerPkt packet) { }

        [PacketHandler(Opcode.CMSG_REQUEST_FORCED_REACTIONS)]
        void HandleRequestForcedReactions(RequestForcedReactionsPkt packet) { }
        ...
        void HandleBattlePetRequestJournal(BattlePetRequestJournalPkt packet) { }

        [PacketHandler(Opcode.CMSG_GM_TICKET_GET_CASE_STATUS)]
        void HandleGmTicketGetCaseStatus(GmTicketGetCaseStatusPkt packet) { }

        [PacketHandler(Opcode.CMSG_TUTORIAL_FLAG)]
        void HandleTutorialFlag(TutorialPkt packet) { }

        [PacketHandler(Opcode.CMSG_REQUEST_CEMETERY_LIST)]
        void HandleRequestCemeteryList(RequestCemeteryListPkt packet) { }

        [PacketHandler(Opcode.CMSG_CLOSE_INTERACTION)]
        void HandleCloseInteraction(CloseInteractionPkt packet) { }
    }
}
