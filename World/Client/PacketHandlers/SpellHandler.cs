...
namespace HermesProxy.World.Client
{
    public partial class WorldClient
    {
        ...
        void HandleAuraUpdateAll(WorldPacket packet)
        {
            if (!LegacyVersion.AddedInVersion(ClientVersionBuild.V3_0_2_9056))
                return; // Only for WotLK

            WowGuid128 guid = packet.ReadPackedGuid().To128(GetSession().GameState);
            AuraUpdate update = new AuraUpdate(guid, true);
        ...
        private void ReadSingleAura(WorldPacket packet, WowGuid128 guid, AuraUpdate update)
        {
            ...
            data.CastID = WowGuid128.Create(HighGuidType703.Cast, SpellCastSource.Aura,
                (uint)GetSession().GameState.CurrentMapId, spellId, guid.GetCounter());
            ...
            // Convert legacy flags to modern flags
            data.Flags = 0;
            data.ActiveFlags = 0;
            if ((flags & 0x10) != 0) // POSITIVE
                data.Flags |= AuraFlagsModern.Positive;
            if ((flags & 0x20) != 0) // DURATION
                data.Flags |= AuraFlagsModern.Duration;

            // Active effects
            if ((flags & 0x01) != 0) data.ActiveFlags |= 1; // EFFECT_0
            if ((flags & 0x02) != 0) data.ActiveFlags |= 2; // EFFECT_1
            if ((flags & 0x04) != 0) data.ActiveFlags |= 4; // EFFECT_2

            // Caster GUID - in WotLK, if NOT_CASTER (0x08) is NOT set, caster guid follows
            if ((flags & 0x08) == 0)
                data.CastUnit = packet.ReadPackedGuid().To128(GetSession().GameState);
            else
                data.CastUnit = guid; // Self-cast
            ...
            // Effect base points (SCALABLE flag = 0x40)
            if ((flags & 0x40) != 0)
            {
                if ((flags & 0x01) != 0) data.Points.Add(packet.ReadFloat());
                if ((flags & 0x02) != 0) data.Points.Add(packet.ReadFloat());
                if ((flags & 0x04) != 0) data.Points.Add(packet.ReadFloat());
            }
