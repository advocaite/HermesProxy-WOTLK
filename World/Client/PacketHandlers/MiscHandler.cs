...
namespace HermesProxy.World.Client
{
    public partial class WorldClient
    {
        ...
        // WotLK 3.3.5a: SMSG_POWER_UPDATE - forward power changes
        ...
        // WotLK 3.3.5a: SMSG_UPDATE_TALENT_DATA
        ...
        void HandleUpdateTalentData(WorldPacket packet)
        {
            ...
            // Skip the legacy talent data - send empty talent info to modern client
            // The modern client uses a different talent system
        }

        // WotLK 3.3.5a: SMSG_ALL_ACHIEVEMENT_DATA
        ...
        void HandleAllAchievementData(WorldPacket packet)
        {
            ...
            // We already send our own stub during login (EmptyAllAchievementData)
            // Just consume and ignore the legacy server's version
        }

        // WotLK 3.3.5a: SMSG_LOAD_EQUIPMENT_SET
        [PacketHandler(Opcode.SMSG_LOAD_EQUIPMENT_SET)]
        void HandleLoadEquipmentSet(WorldPacket packet)
        {
            // Skip - equipment sets have a different format in modern
            // Send empty equipment set to prevent client issues
        }

        // WotLK: SMSG_INSTANCE_DIFFICULTY
        [PacketHandler(Opcode.SMSG_INSTANCE_DIFFICULTY)]
        void HandleInstanceDifficulty(WorldPacket packet)
        {
            // Already handled by MSG_SET_DUNGEON_DIFFICULTY handler
            // Just consume to avoid "unknown packet" warning
        }

        // WotLK: SMSG_LEARNED_DANCE_MOVES - doesn't exist in modern
        [PacketHandler(Opcode.SMSG_LEARNED_DANCE_MOVES)]
        void HandleLearnedDanceMoves(WorldPacket packet)
        {
            // Dance moves don't exist in modern client, just consume
        }

        // WotLK: SMSG_CACHE_VERSION
        [PacketHandler(Opcode.SMSG_CACHE_VERSION)]
        void HandleCacheVersion(WorldPacket packet)
        {
            // Already sent during initial auth, just consume the duplicate from world server
        }
    }
}
