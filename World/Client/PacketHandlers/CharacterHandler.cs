...
namespace HermesProxy.World.Client
{
    public partial class WorldClient
    {
        ...
        void HandleEnumCharactersResult(WorldPacket packet)
        {
            ...
            charEnum.DisabledClassesMask = null; // no class mask

            GetSession().GameState.OwnCharacters.Clear();

            // Characters are parsed normally below
            ...
            for (byte i = 0; i < count; i++)
            {
                ...
                char1.SpecID = 0;
                char1.Unknown703 = 0;
                char1.LastLoginVersion = (uint)Framework.Settings.ClientBuild;
        ...
        {
            // Reset update buffering for new login
            Server.Packets.UpdateObject.ResetLoginBuffer();
            ...
            Framework.Logging.Log.Print(Framework.Logging.LogType.Server, $"[LoginVerifyWorld] Map={verify.MapID} Pos=({verify.Pos.X}, {verify.Pos.Y}, {verify.Pos.Z}) Orient={verify.Pos.Orientation}");
            SendPacketToClient(verify);

            GetSession().GameState.IsInWorld = true;

            // SMSG_INIT_WORLD_STATES - required by 3.4.3 client
            if (ModernVersion.ExpansionVersion >= 3)
            {
                var worldStates = new Server.Packets.EmptyInitWorldStates();
            ...
            // WotLK Classic 3.4.3 requires these packets during login
            if (ModernVersion.ExpansionVersion >= 3)
            {
                SendPacketToClient(new Server.Packets.EmptyAllAchievementData());
                SendPacketToClient(new Server.Packets.EmptyAllAccountCriteria());
                SendPacketToClient(new Server.Packets.EmptySetupCurrency());
                SendPacketToClient(new Server.Packets.EmptySpellHistory());
                SendPacketToClient(new Server.Packets.EmptySpellCharges());
                SendPacketToClient(new Server.Packets.EmptyTalentData());
                SendPacketToClient(new Server.Packets.EmptyActiveGlyphs());
                SendPacketToClient(new Server.Packets.EmptyEquipmentSetList());
                SendPacketToClient(new Server.Packets.EmptyAccountMountUpdate());
                SendPacketToClient(new Server.Packets.EmptyAccountToyUpdate());
                SendPacketToClient(new Server.Packets.EmptyAccountHeirloomUpdate());
                SendPacketToClient(new Server.Packets.BattlePetJournalLockAcquired());

                // SMSG_PHASE_SHIFT_CHANGE - CRITICAL: client crashes without phasing initialization
                var phaseShift = new Server.Packets.PhaseShiftChange();
        ...
        void HandleUpdateActionButtons(WorldPacket packet)
        {
            ...
            // Forward action buttons to modern client
            UpdateActionButtons updateButtons = new();
            updateButtons.ActionButtons = buttons;
            updateButtons.Reason = 0; // Initial
