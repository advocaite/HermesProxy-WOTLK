    ...
    public partial class WorldClient
    {
        ...
        private async Task ReceiveLoop()
        {
            try
            {
                while (true)
                {
                    ...
                    Log.PrintNet(LogType.Debug, LogNetDir.S2P, $"Raw header bytes: {BitConverter.ToString(headerBuffer)}");
                    if (_worldCrypt != null)
                    ...
                    Log.PrintNet(LogType.Debug, LogNetDir.S2P, $"Decoded header: size={packetSize}, opcode={header.Opcode} (0x{header.Opcode:X4}), crypt={(_worldCrypt != null ? "ON" : "OFF")}");
        ...
        private void SendPacket(WorldPacket packet)
        {
            ...
            try
            {
                ...
                // Log raw header before encryption
                Log.PrintNet(LogType.Debug, LogNetDir.P2S, $"Raw header ({headerArray.Length} bytes): {BitConverter.ToString(headerArray, 0, Math.Min(headerArray.Length, 6))}");
                if (_worldCrypt != null)
                ...
                Log.PrintNet(LogType.Debug, LogNetDir.P2S, $"Total bytes on wire: {finalData.Length}, first 16: {BitConverter.ToString(finalData, 0, Math.Min(finalData.Length, 16))}");
        ...
        private void SendPacketToClientDirect(ServerPacket packet)
        {
            ...
            {
                Log.PrintNet(LogType.Warn, LogNetDir.P2C, $"Dropping {packet.GetUniversalOpcode()} - session/gamestate not ready");
                return;
            }
            ...
            if (packet.GetConnection() == ConnectionType.Realm)
            {
                ...
                {
                    Log.PrintNet(LogType.Warn, LogNetDir.P2C, $"Queuing {packet.GetUniversalOpcode()} - RealmSocket not ready yet");
                    lock (pendingPackets)
                    {
